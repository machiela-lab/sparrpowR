...
#
# Notes:
# A) 4/12/20 (IB) - Combines rand_cascon() and rand_srr() functions
# ------------------------------------------ #
...
sparrpowR <- function(x_case, y_case,
  ...
  # Progress bar
  message("Step One: Generating Simulated Data")
  ...
  # Progress bar
  message("\nStep Two: Estimating Spatial Relative Risk")
  
  # Iteratively calculate the log relative risk and asymptotic p-value surfaces
  out_par <- foreach::foreach(k = 1:length(pppList), 
                              .combine = comb, 
                              .multicombine = TRUE, 
                              .packages = "sparr",
    ...
    obs_lrr <- sparr::risk(pppList[[k]], tolerate = T, ...)
