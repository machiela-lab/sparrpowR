...
```{r packages}
loadedPackages <- c("broom", "geojsonio", "ggmap", "ggplot2", "grDevices", "maptools", "raster", "rgdal", "rgeos", "sf", "sp", "sparrpowR", "spatstat", "spatstat.geom", "tibble")
...
sp::plot(spf, col = "magenta", add = T)
legend("bottom", xpd = T, y.intersp = -1.5, legend = c("Navy Yard"), col = "magenta", pch = 3, bty = "n")
```
...
The process above took about `r round(time_srr, digits = 1)` minutes to run. Of the `r length(sim_power$n_con)` iterations, we simulated `r mean(sim_power$n_cas)` case locations and an average `r round(mean(sim_power$n_con))` (SD: `r round(sd(sim_power$n_con), digits = 2)`) control locations for an average prevalence of `r round(mean(sim_power$n_cas/sim_power$n_con)*100, digits = 2)`% (SD: `r round(sd(sim_power$n_cas/sim_power$n_con)*100, digits = 2)`%). The averaged bandwidth for the statistic was `r round(mean(sim_power$bandw)/1000, digits = 1)` kilometers (SD: `r sd(sim_power$bandw)/1000`). We plot the statistical power for a one-tailed, lower-tail hypothesis (`cascon = FALSE`) at `alpha = 0.05` using the `spatial_plots()` function. 
...
Now, lets overlay our results on top of a basemap. Here, we will use an open-source map from [Stamen](http://maps.stamen.com/#terrain/12/37.7706/-122.3782), that is unprojected in WGS84. We extract the rectangular box (i.e., bounding box) surrounding our polygon boundary of the District of Columbia (WGS84). 
...
raster::crs(sim_pts) <- "+init=EPSG:32618" # set initial projection
