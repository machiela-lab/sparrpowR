...
```{r packages, echo=TRUE, warning=FALSE, message=FALSE, cache=TRUE}
loadedPackages <- c("broom", "geojsonio", "ggmap", "ggplot2", "maptools",  "raster", "rgdal", "rgeos", "sp", "sparrpowR","spatstat", "tibble")
invisible(lapply(loadedPackages, require, character.only = TRUE))
set.seed(1234) # for reproducibility
rgeos::set_RGEOS_CheckValidity(0L) # do not check union validity (use with caution)
```
...
```{r echo = TRUE, warning=FALSE, message=FALSE, cache=TRUE}
dcp <- sp::spTransform(dcc, sp::CRS("+init=EPSG:32618"))
...
sim_pts_wgs84 <-  sp::spTransform(sim_pts, sp::CRS("+init=epsg:4326")) # project to basemap
